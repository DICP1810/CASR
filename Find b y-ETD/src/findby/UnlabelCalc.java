...
public class UnlabelCalc {
    ...
    public UnlabelCalc(File mgffile,List<String> scansummary0,int style,List<Spectrum> hcdSpectra){
        expPeptide=new ReadScanSummary(scansummary0,style);
        expPeptide.findExpMSMSPD(hcdSpectra);
    ...
    public void writeAllIonMascot(File outputFile){
         try{
             ...
             FileWriter outFile = new FileWriter(outputFile);
                ...
                out.println("Sequence"+"\t"+"scanFile"+"\t"+"Ion Name"+"\t"+"Ion Number"+"\t"+"ion name"+"\t"+"ion count"+"\t"+"mz"+"\t"+"cid scan");
                ...
                for(int i=0;i<line;i++){
                    ...
                    if(ionNames.get(i).size()>0){
                       ...
                       //if(ionNames.get(i).size()>=3){
                           ...
                           out.println(expPeptide.scanSummary.get("Sequence").get(i)+"\t"+expPeptide.scanSummary.get("scanFile").get(i)+"\t"+ionNames.get(i)+"\t"+Integer.toString(ionNames.get(i).size())+"\t"+name+"\t"+count+"\t"+mz+"\t"+HCD_CID.get(Integer.parseInt(scans)));
                           allResult.add(expPeptide.scanSummary.get("Sequence").get(i)+"\t"+expPeptide.scanSummary.get("scanFile").get(i)+"\t"+ionNames.get(i)+"\t"+Integer.toString(ionNames.get(i).size())+"\t"+name+"\t"+count+"\t"+mz+"\t"+HCD_CID.get(Integer.parseInt(scans)));
                       }
                       else{
...
//should be adjusted according to the headline of input file
        ...
        for(int i=0;i<n;i++){
            if(pepVarMods.get(i).length()>4){
                ...
                //in case unknown amino acid exists and the scan number exists in the expmsms
                if(pepSeq.get(i).indexOf('X')==-1&&expPeptide.expMSMS.containsKey(pepDFile)){
