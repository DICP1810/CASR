...
public class ReadScanSummary {
 ...
 public void findExpMSMSPD(List<Spectrum> hcdSpectra){
   expMSMS.clear();
   for(int i=0;i<hcdSpectra.size();i++){
     HashMap<Double,Double> oneSpectrum=hcdSpectra.get(i).mzIntens;
   ...
   List<Map.Entry<Double,Double>> entries = new ArrayList<Map.Entry<Double,Double>>(oneSpectrum.entrySet());
                                ...
                                expMSMS.put(String.valueOf(hcdSpectra.get(i).scan),orderedSpectrum);
   }
 }
   public void findExpMSMSMascot(File mgffile,HashMap<String,String> ScanSpectrum){
        expMSMS.clear();
        expMSMSBegin.clear();
        expMSMSEnd.clear();
        mgfFile=mgffile;
        ...
        Set<String> spectrum=ScanSpectrum.keySet();
        ...
        try{
            ...
            while ((dataRow=MGFFile.readLine())!=null){
                ...
                if(dataRow.indexOf("TITLE")!=-1)
                {
                    ...
                    if(spectrum.contains(temp[1])){
                        dtaFileName=ScanSpectrum.get(temp[1]);
                        //System.out.println(dtaFileName);
                        scanOrder.put(dtaFileName,n);
                        //the first line of msms
                        expMSMSBegin.put(dtaFileName,lineNumber+5);
                        ...
                        if(data.charAt(0)=='C'){
                            ...
                            //for pd mgf format
                            dataArray=data.split("=");
                            dataArray1=dataArray[1].split(" ");
                            expMass.put(dtaFileName,Double.parseDouble(dataArray1[0]));
                            data=MGFFile.readLine();
                            String cha=String.valueOf(data.charAt(7));
                            expCharge.put(dtaFileName,Integer.parseInt(cha));
                            data=MGFFile.readLine();
