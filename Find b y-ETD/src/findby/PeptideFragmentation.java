...
public class PeptideFragmentation {
    ...
    private boolean aIonSwitch=false;
    ...
    //consider double ion will remove many ions if consider the overlap between different ions and remove these overlapping theoretical ions
    //will remove many a1 ions? even if don't consider overlap between one ion isotope with another ion?why?
    private boolean doubleFragIon=false;
    private boolean motherIonSwitch=false;
    private boolean nthIonSwitch=false;     
    private boolean immoniumIon=false;
    public HashMap<String,Double> theoMSMS=new LinkedHashMap<>();
    ...
    public void calculateMSMS(String peptidesequence,String peptidevarmod,int chargeprecursor){
        ...
        try{
            ...
            calculateBIon();
            System.out.println(chargeprecursor);
            calculatePepMass();
            theoMSMS.clear();
            chargeMS.clear();
            calculateMotherIon();
    ...
    public void calculateBIon()throws NoAminoAcidException{
        ...
        double nTerminalModMass=17.0265;
    ...
    public void calculatePepMass(){
        ...
        //for etd, should minus 17.0265
        peptideMass=-17.0265+bIon.get(bIon.size()-1)+aminoAcidResidueMolWeight.get(peptideSequence.charAt(peptideSequence.length()-1))+lastAminoAcidModMass+cTerminalModMass+waterMW;    
    ...
    public void calculateOtherIon()
    {
        ...
        if(bIonSwitch){
            for(int i=0;i<length;i++){
                ...
                chargeMS.add(1);
                System.out.println("b"+Integer.toString(i+1)+"+");
                System.out.println(temp);
                lossWater=peptideSequence.substring(0,i+1).contains("S")||peptideSequence.substring(0,i+1).contains("T")||peptideSequence.substring(0,i+1).contains("E")||peptideSequence.substring(0,i+1).contains("D");
                if(lossWaterSwitch&&lossWater){
                    theoMSMS.put("b"+Integer.toString(i+1)+"+"+"LossWater",temp-waterMW);
                    chargeMS.add(1);
                }
                lossAmmonia=peptideSequence.substring(0,i+1).contains("R")||peptideSequence.substring(0,i+1).contains("K")||peptideSequence.substring(0,i+1).contains("N");
                if(lossAmmoniaSwitch&&lossAmmonia){
                    theoMSMS.put("b"+Integer.toString(i+1)+"+"+"LossAmmonia",temp-ammoniaMW);
                    chargeMS.add(1);
                }
            }
            //use double b ion always here,but not for other ions with loss of ammonia or water
            if(chargePrecursor>=2){
                for(int i=0;i<length;i++){
                    ...
                    lossWater=peptideSequence.substring(0,i+1).contains("S")||peptideSequence.substring(0,i+1).contains("T")||peptideSequence.substring(0,i+1).contains("E")||peptideSequence.substring(0,i+1).contains("D");
                    if(lossWater&&lossWaterSwitch&&temp-waterMW>doubleIonThres){
                        theoMSMS.put("b"+Integer.toString(i+1)+"++"+"LossWater",(temp-waterMW)/2.0);
                        chargeMS.add(2);
                    }
                    lossAmmonia=peptideSequence.substring(0,i+1).contains("R")||peptideSequence.substring(0,i+1).contains("K")||peptideSequence.substring(0,i+1).contains("N");
                    if(lossAmmonia&&lossAmmoniaSwitch&&temp-ammoniaMW>doubleIonThres){
        ...
        if(yIonSwitch){
            for(int i=0;i<length;i++){
                ...
                System.out.println(temp);
                chargeMS.add(1);
                lossWater=peptideSequence.substring(peptideSequence.length()-i-1).contains("S")||peptideSequence.substring(peptideSequence.length()-i-1).contains("T")||peptideSequence.substring(peptideSequence.length()-i-1).contains("E")||peptideSequence.substring(peptideSequence.length()-i-1).contains("D");
                if(lossWater&&lossWaterSwitch){
                    theoMSMS.put("y"+Integer.toString(i+1)+"+"+"LossWater",temp-waterMW);
                    chargeMS.add(1);
                }
                lossAmmonia=peptideSequence.substring(peptideSequence.length()-i-1).contains("R")||peptideSequence.substring(peptideSequence.length()-i-1).contains("K")||peptideSequence.substring(peptideSequence.length()-i-1).contains("N");
                if(lossAmmonia&&lossAmmoniaSwitch){
        ...
        //add immonium ion-对标记定量没有用,因为质量相同,对无标记定量有用
        if(immoniumIon){
            for(int i=0;i<aminoAcidResMolWeight.length;i++){
                theoMSMS.put("immonium"+aminoAcidAbbreviation.charAt(i),aminoAcidResMolWeight[i]-coMW+protonMolWeight);
                chargeMS.add(1);
            }
        }
